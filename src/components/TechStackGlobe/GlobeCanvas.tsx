...
function iconShadow(selected: boolean, hovered: boolean): string {
  if (selected) return '0 4px 16px rgba(0,0,0,0.15),0 0 0 2px rgba(179,125,34,0.9)';
  if (hovered) return '0 8px 24px rgba(0,0,0,0.2),0 0 0 1.5px rgba(179,125,34,0.7)';
...
export default function GlobeCanvas({
  ...
  const draw = useCallback((now: number) => {
    ...
    for (const [a, b] of sphere.edges) {
      ...
      ctx.strokeStyle = `rgba(179,125,34,${edgeBase + alpha * edgeRange})`;
      ctx.lineWidth = 0.8;
      ctx.stroke();
    }
    ...
    >
          ...
          >
            <div style={{
              ...
              {!imgErrors.has(errKey) ? (
                ...
                <span style={{ fontFamily: 'monospace', fontSize: '0.6rem', fontWeight: 700, color: '#1a120b', letterSpacing: '0.04em' }}>
      ...
      {tooltip && !frozen && (
        <div style={{
          ...
          background: '#ffffff', border: '1px solid var(--border-color, #e2d9c9)',
          borderRadius: 8, padding: '4px 10px', pointerEvents: 'none', zIndex: 200, whiteSpace: 'nowrap',
          boxShadow: '0 4px 15px rgba(26, 18, 11, 0.05)'
        }}>
          <span style={{ fontSize: '0.75rem', fontWeight: 600, color: '#1a120b' }}>
            {tooltip.name}
          </span>
        </div>
      )}
